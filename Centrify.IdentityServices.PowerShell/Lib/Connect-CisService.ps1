...
.OUTPUTS
This Cmdlet returns a CisConnection object that print information about the opened connection to the Centrify Identity Service.

.EXAMPLE
C:\PS> Connect-CisService -Url cps.ocean.net -User admin@cps.ocean.net
...
.EXAMPLE
C:\PS> Connect-CisService -Url cps.ocean.net -UseMachineCertificate -Thumbprint "009D5C993F3ABDE6B5198924CC39B48B4B90C5EE"
...
.EXAMPLE
C:\PS> Connect-CisService -Url cps.ocean.net -OAuth2Service "Bluecrab" -OAuth2Scope "All" -OAuth2Secret "c3ZjLWJjcmFiQGNwcy5vY2Vhbi5uZXQ6Q2VudHIxZnk="
...
$Secret = Centrify.IdentityServices.PowerShell.OAuth2.ConvertToSecret -ClientID "svc-bcrab@cps.ocean.net" -Password "Centr1fy"
#>
function global:Connect-CisService
{
	param
	(
	...
	{	
        ...
        $global:CisConnection = [Void]$null

        if ($UseUserCertificate.IsPresent)
		{
			# Get User Certificate from CurrentUser store
			$X509Certificate = Centrify.IdentityServices.PowerShell.X509Certificates.GetCertificateFromStore -StoreName "CurrentUser" -Thumbprint $Thumbprint
			...
			$WebResponse = Invoke-WebRequest -Method Post -SessionVariable CisSession -Uri $Uri -Body $Json -ContentType $ContentType -Headers $Header -Certificate $X509Certificate -TimeoutSec 60
            $WebResponseResult = $WebResponse.Content | ConvertFrom-Json
            if ($WebResponseResult.Success)
		    {
                # Get Session Token from successfull login
			    Write-Debug ("WebResponse=`n{0}" -f $WebResponseResult)
			    # Validate that a valid .ASPXAUTH cookie has been returned for the CisConnection
			    $CookieUri = ("https://{0}" -f $Url)
			    $ASPXAuth = $CisSession.Cookies.GetCookies($CookieUri) | Where-Object { $_.Name -eq ".ASPXAUTH" }
			    ...
			    else
			    {
				    ...
				    $Connection | Add-Member -MemberType NoteProperty -Name Session -Value $CisSession

				    # Set Connection as global
				    $global:CisConnection = $Connection
		...
		elseif ($UseMachineCertificate.IsPresent)
		{
			...
			$X509Certificate = Centrify.IdentityServices.PowerShell.X509Certificates.GetCertificateFromStore -StoreName "LocalMachine" -Thumbprint $Thumbprint
		...
		elseif (-not [System.String]::IsNullOrEmpty($OAuth2Service))
        {
			...
			$BearerToken = Centrify.IdentityServices.PowerShell.OAuth2.GetBearerToken -Url $Url -Service $OAuth2Service -Secret $OAuth2Secret -Scope $OAuth2Scope
			...
			# Connect using Certificate
			$WebResponse = Invoke-WebRequest -Method Post -SessionVariable CisSession -Uri $Uri -Body $Json -ContentType $ContentType -Headers $Header
            $WebResponseResult = $WebResponse.Content | ConvertFrom-Json
            if ($WebResponseResult.Success)
		    {
				# Get Connection details
        ...
        else
		{
			...
			# Initiate connection
			$InitialResponse = Invoke-WebRequest -Method Post -SessionVariable CisSession -Uri $Uri -Body $Json -ContentType $ContentType -Headers $Header
