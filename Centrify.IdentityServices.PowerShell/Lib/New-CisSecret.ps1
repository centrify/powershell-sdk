...
<#
.SYNOPSIS
This CMDlet supports ability to create new [Object] CisSecret of type Text, OR File. 
.
.DESCRIPTION
This CMDlet supports ability to create new [Object] CisSecret of type Text, OR File. 
...
.OUTPUTS
Returns the CisSecret object on success. Returns failure message on failure.

.EXAMPLE
C:\PS> New-CisSecret -Name "Ocean" -Text "The ocean is blue"
Creates a new secret named 'ocean' with the specified text.

.EXAMPLE
C:\PS> New-CisSecret -File "C:\tmp\file.txt" -Name "Ocean2" 
Creates a new secret named 'Ocean2' with the specified text.
#>
function global:New-CisSecret
{
	...
	{	
		...
		$Uri = ("https://{0}/ServerManage/AddDataVaultItem" -f $CisConnection.PodFqdn)
        ...
        elseif (-not [System.String]::IsNullOrEmpty($File))
        {
            ...
            $UploadRequest = Centrify.IdentityServices.PowerShell.DataVault.RequestSecretUploadUrl -Name $FileInfo.Name -Size $FileInfo.Length -SecretID $null
