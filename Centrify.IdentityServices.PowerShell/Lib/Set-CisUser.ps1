...
<#
.SYNOPSIS
This Cmdlet sets important attributes on an existing CIS user.
...
.PARAMETER CisUser
Mandatory parameter to specify the CisUser to set Attributes to
...
.EXAMPLE
C:\PS> Set-CisUser -CisUser (Get-CisUser -Filter "bcrab")

.EXAMPLE
C:\PS> Set-CisUser -CisUser (Get-CisUser -Filter "bcrab") -MobileNumber "5555555555"
Updates the MobileNumber attribute for CisUser "bcrab" 
#>
function global:Set-CisUser
{
	param
	(
		[Parameter(Mandatory = $true, ValueFromPipeline = $true, HelpMessage = "Specify the CisUser to set Attributes to.")]
	...
	{	
		...
		# Get CISUser Attributes
		$CloudUser = Get-CisUserAttributes -CISUser $CisUser
		if ($CloudUser -eq [Void]$null)
		{
			Throw "Unable to get CISUser informations"
		}
		...
		{
			$LoginName = $CISUser.UserName.Split('@')[0]
		}
		if ([System.String]::IsNullOrEmpty($LoginSuffix))
		{
			$LoginSuffix = $CISUser.UserName.Split('@')[1]
		}
		...
		$Uri = ("https://{0}/CDirectoryService/ChangeUser" -f $CisConnection.PodFqdn)
		$ContentType = "application/json" 
		$Header = @{ "X-CENTRIFY-NATIVE-CLIENT" = "1"; }

		# Format Json query
		$Json = "{"
		if ($CISUser.SourceDsType -eq "CDS")
		{
