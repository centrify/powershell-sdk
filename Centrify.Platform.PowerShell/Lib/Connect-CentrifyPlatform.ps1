...
.EXAMPLE
C:\PS> Connect-CentrifyPlatform -Url cps.ocean.net -User admin@cps.ocean.net
...
.EXAMPLE
C:\PS> Connect-CentrifyPlatform -Url cps.ocean.net -UseMachineCertificate -Thumbprint "009D5C993F3ABDE6B5198924CC39B48B4B90C5EE"
...
.EXAMPLE
C:\PS> Connect-CentrifyPlatform -Url cps.ocean.net -Client "Bluecrab" -Scope "All" -Secret "c3ZjLWJjcmFiQGNwcy5vY2Vhbi5uZXQ6Q2VudHIxZnk="
...
C:\PS> Connect-CentrifyPlatform -Url cps.ocean.net -EncodeSecret
#>
function global:Connect-CentrifyPlatform
{
	param
	(
	...
	{	
        ...
        if ($UseUserCertificate.IsPresent)
		{
			...
			$X509Certificate = Centrify.Platform.PowerShell.X509Certificates.GetCertificateFromStore -StoreName "CurrentUser" -Thumbprint $Thumbprint
			...
			$WebResponse = Invoke-WebRequest -UseBasicParsing -Method Post -SessionVariable PASSession -Uri $Uri -Body $Json -ContentType $ContentType -Headers $Header -Certificate $X509Certificate -TimeoutSec 60
		...
		elseif ($UseMachineCertificate.IsPresent)
		{
			...
			$X509Certificate = Centrify.Platform.PowerShell.X509Certificates.GetCertificateFromStore -StoreName "LocalMachine" -Thumbprint $Thumbprint
		...
		elseif (-not [System.String]::IsNullOrEmpty($Client))
        {
			...
			$BearerToken = Centrify.Platform.PowerShell.OAuth2.GetBearerToken -Url $Url -Client $Client -Secret $Secret -Scope $Scope
        ...
        elseif ($EncodeSecret.IsPresent)
        {
            ...
            return ("Secret: {0}" -f (Centrify.Platform.PowerShell.OAuth2.ConvertToSecret -Client $Client -Password $Password))
        }		
        elseif ($DecodeSecret.IsPresent)
        {
            ...
            return (Centrify.Platform.PowerShell.OAuth2.ConvertFromSecret -Secret $Secret)
        }		
        else
		{
