...
<#
.SYNOPSIS
This CMDlet supports ability to create a new PASUser
...
.EXAMPLE
C:\PS> New-CentrifyUser -Login "bcrab" -LoginSuffix "ocean.net" -DisplayName "Eugene Harold Krabs" -Password "BBadsfsd90*()" -Mail "bcrab@ocean.net"
Create a new use 'bcrab' with the mandatory parameters
#>
function global:New-CentrifyUser
{
	param
	(
	...
	{	
		...
		$Uri = ("https://{0}/cDirectoryService/CreateUser" -f $PlatformConnection.PodFqdn)
		...
		{
			# Success return User
			return (Get-CentrifyUser | Where-Object { $_.UserName -eq $UserName })
		}
		else
		{
			# Query error
