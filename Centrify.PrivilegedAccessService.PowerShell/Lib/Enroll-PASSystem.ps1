...
.EXAMPLE
C:\PS> Enroll-PASSystem -Url "vault.centrify.lab" -EnrollmentCode "2EE9F1CA-F06A-4470-B07A-32907444A023"
Enrolls a System to Centrify Platform using the tenant URL and enrollment code.
#>
function global:Enroll-PASSystem
{
	param
	(
	...
	{	
		...
		# Create PASSystem
		$PASSystem = @{}
		$PASSystem.Name					= $ResourceName
		$PASSystem.ResourceName			= $ResourceName
		$PASSystem.FQDN					= $FQDN
		$PASSystem.OperatingSystem 		= "Windows"
		$PASSystem.AgentVersion 			= "1.0.0" 
		$PASSystem.EnrollmentCode	 		= $EnrollmentCode
		$PASSystem.CertificatePassword	= $CertificatePassword
		...
		if ($WebResponseResult.Success)
		{
			...
			$X509Certificate = Centrify.PrivilegedAccessService.PowerShell.X509Certificates.GetCertificateFromBase64String -Base64String $WebResponseResult.Result.Cert -Password $Password
			...
			Centrify.PrivilegedAccessService.PowerShell.X509Certificates.AddCertificateToStore -Store "Cert:\LocalMachine\My"  -Certificate $X509Certificate
			
			# Return enrolled resource
			return (Get-PASSystem -Name -eq $Name)
		}
		else
		{
