...
.EXAMPLE
C:\PS> New-PASUser -Login "bcrab" -LoginSuffix "ocean.net" -DisplayName "Eugene Harold Krabs" -Password "BBadsfsd90*()" -Mail "bcrab@ocean.net"
Create a new use 'bcrab' with the mandatory parameters
#>
function global:New-PASUser
{
	param
	(
	...
	{	
		...
		$Uri = ("https://{0}/cDirectoryService/CreateUser" -f $PASConnection.PodFqdn)
		...
		{
			# Success return User
			return (Get-PASUser | Where-Object { $_.UserName -eq $UserName })
		}
		else
		{
			# Query error
