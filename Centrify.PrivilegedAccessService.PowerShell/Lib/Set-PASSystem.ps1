...
NOTE: Get-PASSystem commandlet must be used to acquire the PASSystem object to update
...
NOTE: Get-PASSystem commandlet must be used to acquire the object
...
.EXAMPLE
C:\PS> Set-PASSystem -PASSystem (Get-PASSystem -Name "Windows7") -Name "Windows7Sys1"
This CmdLet gets the specified system and updates the name.
#>
function global:Set-PASSystem
{
	param
	(
	...
	{	
		...
		else
		{
			...
			{
				$ComputerClass = $PASSystem.ComputerClass
			}
			if ([System.String]::IsNullOrEmpty($SessionType))
			{
				$SessionType = $PASSystem.SessionType
			}
			if (-not $Port)
			{
				$Port = $PASSystem.Port
			...
			{
				$Description = $PASSystem.Description
			}
			if ([System.String]::IsNullOrEmpty($ProxyUser))
			{
				$ProxyUser = $PASSystem.ProxyUser
			}
			if ([System.String]::IsNullOrEmpty($ProxyUserPassword))
			{
				$ProxyUserPassword = $PASSystem.ProxyUserPassword
			}
			if (-not $ProxyUserIsManaged)
			{
				$ProxyUserIsManaged = $PASSystem.ProxyUserIsManaged
			}
			if ([System.String]::IsNullOrEmpty($ManagementMode))
			{
				$ManagementMode = $PASSystem.ManagementMode
			}
			if (-not $ManagementPort)
			{
				$ManagementPort = $PASSystem.ManagementPort
			}
			if (-not $AllowPasswordRotation)
			{
				$AllowPasswordRotation = $PASSystem.AllowPasswordRotation
			}
			if ([System.String]::IsNullOrEmpty($PasswordRotationDuration))
			{
				$PasswordRotationDuration = $PASSystem.PasswordRotationDuration
			}
			if (-not $AllowMultipleCheckouts)
			{
				$AllowMultipleCheckouts = $PASSystem.AllowMultipleCheckouts
			}
			if (-not $AllowPasswordHistoryCleanup)
			{
				$AllowPasswordHistoryCleanup = $PASSystem.AllowPasswordHistoryCleanup
			}
			if ([System.String]::IsNullOrEmpty($PasswordHistoryCleanupDuration))
			{
				$PasswordHistoryCleanupDuration = $PASSystem.PasswordHistoryCleanupDuration
			}
			if (-not $AllowHealthCheck)
			{
				$AllowHealthCheck = $PASSystem.AllowHealthCheck
			}
			if ([System.String]::IsNullOrEmpty($HealtCheckInterval))
			{
				$HealtCheckInterval = $PASSystem.HealtCheckInterval
			}
			if ([System.String]::IsNullOrEmpty($DefaultCheckoutTime))
			{
				$DefaultCheckoutTime = $PASSystem.DefaultCheckoutTime
			}
			if ([System.String]::IsNullOrEmpty($ProxyCollectionList))
			{
				$ProxyCollectionList = $PASSystem.ProxyCollectionList
			}
			
			# Setup variable for query
			$Uri = ("https://{0}/ServerManage/UpdateResource" -f $PASConnection.PodFqdn)
			$ContentType = "application/json" 
			$Header = @{ "X-CENTRIFY-NATIVE-CLIENT" = "1"; }
	
			# Format Json query
			$Json = "{"
			$Json += ("`"ID`":`"{0}`"," -f $PASSystem.ID)
			$Json += ("`"Name`":`"{0}`"," -f $Name)
			$Json += ("`"FQDN`":`"{0}`"," -f $Fqdn)
			...
			if ($WebResponseResult.Success)
			{
				...
				return (Get-PASSystem | Where-Object { $_.ID -eq $PASSystem.ID })
			}
			else
			{
				# Query error
