...
function Centrify.PrivilegedAccessService.PowerShell.DataVault.RequestSecretUploadUrl
{
	param
	(
	...
	try
	{
		...
		$Uri = ("https://{0}/ServerManage/RequestSecretUploadUrl" -f $PASConnection.PodFqdn)
...
function Centrify.PrivilegedAccessService.PowerShell.DataVault.RequestSecretDownloadUrl
{
	param
	(
	...
	{
		# Setup variable for connection
		$Uri = ("https://{0}/ServerManage/RequestSecretDownloadUrl" -f $PASConnection.PodFqdn)
		$ContentType = "application/json" 
		$Header = @{ "X-CENTRIFY-NATIVE-CLIENT" = "1" }
...
function Centrify.PrivilegedAccessService.PowerShell.DataVault.UploadSecretFile
{
	param
	(
	...
	try
	{
		...
		# Connect using RestAPI
		$WebResponse = Invoke-WebRequest -Method Put -Uri $UploadUrl -Body $Data -ContentType $ContentType -Headers $Header -WebSession $PASConnection.Session
		$WebResponseResult = $WebResponse.Content | ConvertFrom-Json
		if ($WebResponseResult.Success)
		{
            # Return nothing
...
function Centrify.PrivilegedAccessService.PowerShell.DataVault.DownloadSecretFile
{
	param
	(
	...
	{
		# Connect using RestAPI
		$WebResponse = Invoke-WebRequest -Method Get -Uri $DownloadUrl -WebSession $PASConnection.Session -OutFile $Path
...
function Centrify.PrivilegedAccessService.PowerShell.DataVault.ConvertFileSize
{
	param
	(
...
function Centrify.PrivilegedAccessService.PowerShell.DataVault.GetSecretContent
{
	param
	(
	...
	{
		# Setup variable for connection
		$Uri = ("https://{0}/ServerManage/RetrieveDataVaultItemContents" -f $PASConnection.PodFqdn)
        $ContentType = "application/json" 
		$Header = @{ "X-CENTRIFY-NATIVE-CLIENT" = "1" }
